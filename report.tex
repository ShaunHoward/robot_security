...
noisy input measurements from multiple robots and determine the location of a mobile robot in the view angle of the others.
Further, an Unscented Kalman Filter (UKF) is put to the test in place of the EKF to compare results between the two filters based on the 
same simulation including one moving robot and two stationary robots that watch it. The simulation proves
that both the EKF and UKF are viable solutions for correcting odometry data for at least one robot when
grouped with a squadron of at least one additional robot.


% Not sure this part really makes sense, since we didn't examine/simulate any attack vectors
...
filters. Our overall system structure can be seen 
